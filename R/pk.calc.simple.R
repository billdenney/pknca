...
#'   `NA_real_` if any of the following are true
#'   `is.na(aucinf)`, `is.na(auclast)`, `aucinf <= 0`,
#'   or `auclast <= 0`.
#' @export
...
#' @details If `dose` is the same length as the other inputs, then the
...
#'   inputs other than `dose` are scalars and `dose` is a vector, then
#'   the function assumes multiple doses were given in a single interval, and
#'   the sum of the `dose`s will be used for the calculation.
...
#' kinetics.
#'
#' @details mrt.md is `aumctau/auctau + tau*(aucinf-auctau)/auctau` and
...
#' @details Note that if `aucinf == auctau` (as would be the
...
#' @inheritParams assert_conc_time
#' @inheritParams assert_intervaltime_single
#' @return The concentration when `time == end`.  If none
#'   match, then `NA`
#' @family NCA parameters for concentrations during the intervals
#' @export
...
#' @param cmax The maximum observed concentration
#' @param cmin The minimum observed concentration
#' @return The swing above the minimum concentration.  If `cmin` is zero,
...
#' @param check Run [assert_conc_time()]?
#' @return The concentration at the end of the infusion, `NA` if
